...
holds for all $x\in X,y\in Y$, and $(x^\ast,y^\ast)$ is a saddle-point. Conversely, all saddle-points are critical (as shown in the minimax note, this holds for any function, not just saddle-functions). In addition, all saddle-points $(x^\ast,y^\ast)$ satisfy the minimax equality:

$$\sup_x \inf_y f(x,y) = \inf_y \sup_x f(x,y) = f(x^\ast,y^\ast)$$

Therefore all saddle-points produce the same value of $f$. Conversely, if the point $(x^\ast,y^\ast)$ solves any one of these optimizations, it must be a critical point and hence a saddle-point.
...
Now we consider a minimax problem of the form,

$$\min_x \max_y f(x,y)$$

subject to affine constrains of the form $Ax+By=c$, where $A,B$ are constant matrices of appropriate dimension and $c$ a constant vector. We suppose that $f:X\times Y\rightarrow \mathbb R$ is a saddle-function (convex in $x$ and concave in $y$).

Let $\Omega \subset X \times Y$ be the set of points $(x,y)\in X\times Y$ satisfying the constrain $A x + B y = c$

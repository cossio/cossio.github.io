...
$$f(x) = \sup_y (y^T x - f^*(y))$$
$$f(1) = \sup_y (y^T - f^*(y))$$
$$f(-1) = \sup_y (-y^T - f^*(y)) = -\inf_y (y^T + f^*(y))$$
...
We can also define an analogue of the Legendre transform for concave functions. Let $f(x)$ be a concave differentiable function. Let $g^*(\xi)$ be the Legendre transform of the convex function $g(x) = -f(x)$. Then,
